...
function handleMonitoringEvent(event) {
    publish("monitoring", event.data, true);
}

function handleBreakpointEvent(event) {
    publish("breakpoint", event.data, true);
}

function injectNodeMessage(message) {
    var msg = JSON.parse(message);
    if (msg.type == null || msg.type != 'debug')
        return;

    var nodeDebug = runtime.nodes.getNode(msg.nodeId);
    if (nodeDebug) {
        try {
            msg.msg.payload = JSON.parse(msg.msg.payload);
        } catch (e) {
            msg.msg.payload = JSON.parse(JSON.stringify(msg.msg.payload));
        }

        nodeDebug.receive(msg.msg);
    }
}

function debugSystem(message) {
    var msg = JSON.parse(message);
    if (msg.type == null || msg.type != 'breakpoint')
        return;
    ...
    switch (msg.subType) {
        ...
        case 'step':
            ...
            nodeDebug.consumeMessage(msg.msg);
            break;
        case 'clear':
            nodeDebug.clearQueue();
            break;
    }
}

function disableDebug() {
    runtime.nodes.eachNode(function (node) {
        if (!node) return;
        var n = runtime.nodes.getNode(node.id);
...
var api = module.exports = {
    init: function(_runtime) {
        ...
        runtime.events.removeListener("monitoring",handleMonitoringEvent);
        runtime.events.on("monitoring",handleMonitoringEvent);

        runtime.events.removeListener("breakpoint",handleBreakpointEvent);
        runtime.events.on("breakpoint",handleBreakpointEvent);
    },
    ...
    */
    addConnection: function(opts) {
        ...
        opts.client.ws.on('message', injectNodeMessage);
        opts.client.ws.on('message', debugSystem);
