...
Node.prototype.send = function(msg) {
    ...
    if (msg === null || typeof msg === "undefined") {
        ...
        if (this._wire) {
            ...
            sendMonitoringData(this.id, msg, "output");
...
Node.prototype.receive = function (msg) {
    ...
    msg.receivedTimestamp = new Date();

    if (this.debug) {
        if (this.messages.length == 0)
            sendMonitoringData(this.id, msg, "input");
...
Node.prototype.toggleDebug = function () {
...
Node.prototype.consumeMessage = function (message) {
    if (this.messages.length == 0) {
        // Send message to alert about empty messages
        sendBreakpointEvents(this.id, "empty_queue");
        return;
    }
    ...
    if (this.messages.length != 0)
        sendMonitoringData(this.id, this.messages[0], "input");
};

Node.prototype.consumeQueue = async function () {
    while (this.messages.length != 0) {
        let frequencyTime = 0;

        let lastTwoMessages = this.messages.slice(0, 2);
        if (lastTwoMessages.length == 2)
            frequencyTime = lastTwoMessages[1].receivedTimestamp - lastTwoMessages[0].receivedTimestamp;

        let msg = this.messages.shift();
        await sleep(frequencyTime);
...
Node.prototype.clearQueue = function () {
    this.messages = [];
};
...
const sendMonitoringData = function(nodeId, msg, type) {
    events.emit("monitoring",{
        data: {
            id: nodeId,
            value: redUtil.cloneMessage(msg),
            type: type
        }
    });
};

const sendBreakpointEvents = function(nodeId, type) {
    events.emit("breakpoint",{
        data: {
            id: nodeId,
            type: type
        }
