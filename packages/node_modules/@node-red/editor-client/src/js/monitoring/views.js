RED.monitoring.view = (function () {
    ...
    function injectHTMLIntoNode(nodeId, toStatus) {
        ...
        iDiv.innerHTML = '<div class="monitoring-toggle"><span>Monitor: </span>' +
            '<input style="float: right; margin-right: 0.3em; margin-top: 0.2em;" type="checkbox" id="checkbox-' + nodeId + '"></input>' +
            '<i id="config-' + nodeId + '"style="display: none; float: right; margin-right: 0.3em; margin-top: 0.3em;" class="fa fa-cog"></i>' +
            '</div>';
        ...
        iDiv.innerHTML += '<div class="monitoring-data" style="display: none; padding-top: 0.2em;">' +
            ...
            '<div style="display: none;"><span></span><canvas id="chart-' + nodeId + '" width="' + foreign.getAttribute('width') + '" height="150"></canvas></div>' +
            '</div>' +
        ...
        document.getElementById('config-' + nodeId).addEventListener('click', openConfigModal.bind(null, nodeId));
    ...
    function toggleMonitoring(monitoringDiv, event) {
        ...
        if (event.target.checked) {
            event.target.parentNode.querySelector('i').style.display = '';
            nodeDataDiv.style.display = '';
        } else {
            event.target.parentNode.querySelector('i').style.display = 'none';
    ...
    function openInjectMessageModal(nodeId) {
        ...
        document.querySelector('.modal-content button').addEventListener('click', function (event) {
            RED.nodes.sendMessage(nodeId, inputMessage.value, 'debug');
            monitoringModal.style.display = 'none';
        });
    }

    function playPauseBreakpoint(nodeId) {
        RED.nodes.sendMessage(nodeId, {}, 'breakpoint', 'play/pause');
    }


    function stepBreakpoint(nodeId) {
        RED.nodes.sendMessage(nodeId, {}, 'breakpoint', 'step');
    }

    function clearBreakpoint(nodeId) {
        RED.nodes.sendMessage(nodeId, {}, 'breakpoint', 'clear');
    }
