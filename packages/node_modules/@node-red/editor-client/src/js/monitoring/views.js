RED.monitoring.view = (function () {
    const monitoringSizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
            ...
            if (nodeId && entry.borderBoxSize && entry.borderBoxSize.blockSize != 0) {
                const monitoringDetailsForeign = document.getElementById(`monitoringDetails-${nodeId}`);
                monitoringDetailsForeign.setAttribute('height', entry.borderBoxSize.blockSize);
            }
        }
    });
    ...
    function drawGraphic(nodeId) {
        ...
        const values = (Array.isArray(data[0]) ? data[nodeSettings.portNumber] : data);

        const graphicElement = document.getElementById(`chart-${nodeId}`);
