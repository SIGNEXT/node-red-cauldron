RED.monitoring.view = (function () {
    ...
    function injectMonitoringDetails(nodeId) {
        ...
        monitoringDetailsForeign.setAttribute('width', (nodeWidth < 100 ? 130 : nodeWidth));
        ...
        iDiv.innerHTML += '<div class="monitoring-graphics" style="padding-bottom: 0.5em;">' +
            ...
            '<span>Messages:</span>' + '<i id="inject-' + nodeId + '"style="float:right; padding-top: 0.2em; padding-right: 0.3em;" class="fa fa-paper-plane"></i>' + '<i id="expand-' + nodeId + '"style="float:right; padding-top: 0.3em; padding-right: 0.3em;" class="fa fa-external-link"></i>' +
        ...
        if(RED.nodes.node(nodeId).inputs == 0) {
            document.getElementById('inject-' + nodeId).style.display = 'none';
        } else {
    ...
    function toggleMonitoring(nodeId) {
        ...
        var monitoringDetailsDiv = nodeDataDiv.querySelector('.monitoring-details');
    ...
    function openInjectMessageModal(nodeId) {
        ...
        content.innerHTML = '<h2>Node: ' + nodeId + '</h2><h3>Inject Message</h3>' +
            '<div><label>Payload: </label><input type="text" value="' + JSON.stringify(lastMessage).replace(/"/g, "'") + '"><button style="margin-left:0.5em;" type="submit">Send!</button></div>';
    ...
    function openLastMessages(nodeId) {
        ...
        RED.monitoring.data[nodeId].output.map(function (outputMessages, outputNumber) {
            var outputDiv = nodeDataDiv.querySelector('.monitoring-values-output');
    ...
    function convertMessageToHTML(message) {
        return '<span>Value: ' + JSON.stringify(message.y) + '</span> - ' + '<span>Time: ' + message.x.toLocaleString() + '-' + message.x.getMilliseconds() + '</span>';
    }

    function toggleMessageToInput(nodeId) {
        var nodeDataDiv = document.getElementById('monitoring-' + nodeId).querySelector('.monitoring-details');
        var valuesDiv = nodeDataDiv.querySelector('.monitoring-values');

        var valueHTMLElement;
        var playPauseButton = document.getElementById('playPause-' + nodeId);
        ...
        if (RED.monitoring.data[nodeId].settings.pauseDebug) {
            ...
            valueHTMLElement = valuesDiv.querySelectorAll('span')[2];
            valueHTMLElement.outerHTML = '<input class="breakpoint-modify-message" type="text" value="' + valueHTMLElement.innerHTML + '" style="width: 90px; height: 25px;"></input>'
        } else {
            ...
            if (valuesDiv.innerHTML == '') return;
            valueHTMLElement = valuesDiv.querySelector('input');
            valueHTMLElement.outerHTML = '<span>' + valueHTMLElement.value + '</span>'
        }
    }
    ...
    function writeMessage(nodeId, timestamp, value) {
        ...
        if (RED.monitoring.data[nodeId].settings.pauseDebug) {
            ...
            valuesDiv.innerHTML += '<span>' + value + '</textarea>';
        }
    }
    ...
    function getMessageInputStep(nodeId) {
        ...
        if (RED.monitoring.data[nodeId].settings.pauseDebug) {
            if (valuesDiv.innerHTML == '') return;
            var input = valuesDiv.querySelector('input');
    ...
    function playPauseBreakpoint(nodeId) {
        ...
        toggleMessageToInput(nodeId);
    }

    function stepBreakpoint(nodeId) {
        var value = getMessageInputStep(nodeId);
        var messageSaved = RED.monitoring.data[nodeId].input.slice(-1)[0];
        var message = {
    ...
    function drawGraphic(nodeId) {
        ...
        var numberOfPointsDisplayed = RED.monitoring.settings.graphicPointsDisplayed;
    ...
    return {
        ...
        clearHTMLNodeMessages: clearHTMLNodeMessages
    }
})();