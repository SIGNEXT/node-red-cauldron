RED.monitoring.view = (function () {
    ...
    function openInjectMessageModal(nodeId) {
        ...
        content.innerHTML = '<h2>Node: ' + nodeId + '</h2><h3>Inject Message</h3>' +
            '<div><label>Payload: </label><input type="text"><button style="margin-left:0.5em;" type="submit">Send!</button></div>';
    ...
    function writeMessage(nodeId, timestamp, value) {
        ...
        if (RED.monitoring.data[nodeId].settings.pauseDebug) {
            valuesDiv.innerHTML += '<input class="breakpoint-modify-message" type="text" value="' + value + '" style="width: 90px; height: 25px;"></input>'
        } else {
            valuesDiv.innerHTML += '<span>' + value + '</span>'
        }
    }
